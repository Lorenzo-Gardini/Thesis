...
\begin{itemize}
    ...
    \item $\gamma$: iper-parametro, il \textit{learning rate} utilizzato per l'algoritmo SGD.
    \item $\lambda$: iper-parametro, corrisponde al fattore di regolarizzazione nella funzione obbiettivo
...
$k$ è un iper-parametro di ciascun algoritmo.
...
\begin{itemize}
    \item ottimizzazione diretta: massimizza la probabilità che un item positivo sia sopra uno negativo.
...
\begin{itemize}
    \item negative sampling critico: la scelta degli item negativi influisce molto sull'apprendimento.
