...
Ogni algoritmo può quindi ricevere in ingresso dati in uno di questi due formati. Questo vincolo vale anche per tutte le altre funzionalità della libreria con alcune eccezioni.
...
\begin{itemize}
    ...
    \item \texttt{InvertibleGroupByFunction}: estende \texttt{GroupByFunction} e \\ \texttt{InverseTransformer}, permette di invertire le trasformazioni su gruppi.
    Vengono definiti i metodi astratti \texttt{\_build\_transform\_function} e \\ \texttt{\_build\_inverse\_function}, applicando il pattern \textit{Template Method}, i quali vengono invocati entrambi dentro i metodi \texttt{fit} e restituiscono una funzione ciascuno che vengono salvate come attributi.
    Queste funzioni vengono poi utilizzate durante l'invocazione dei metodi \texttt{transform} e di\\\texttt{inverse\_transform} per applicare la trasformazione corrispondente.
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.14]{figures/UML/preprocessing/invertible_group_by.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[angle=90, scale=0.15]{figures/UML/preprocessing/direct_1.png}
...
Importante inserire, all'interno del due metodi, un controllo che verifichi che il modello sia stato addestrato, utilizzando la funzione \texttt{check\_is\_fitted} come per il metodo \texttt{predict} descritto in precedenza.
...
\textit{dataset} in \textit{trainset} e \textit{testset}. La principale, chiamata \texttt{train\_test\_split\_with\_coverage}, riceve in ingresso un \textit{DataFrame} o un \textit{NumPy array} che deve rispettare il formato standard definito sui dati che utilizzano i modelli. 
